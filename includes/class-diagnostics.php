...
final class Diagnostics {
	...
	public function report(): array {
		...
		// indices of other systems (ElasticPress, AtoM), whose shards are none of
		...
		if ( 'index' === $shard_scope
			...
			$findings[] = $this->finding(
				...
				sprintf(
					...
					__( 'O índice "%1$s" está íntegro, mas o cluster reporta %2$s — há shards não alocados em índices de outros sistemas (ElasticPress, AtoM etc.). Isso não afeta a busca do Tainacan.', 'tainacan-index-manager' ),
					(string) $snapshot['index_name'],
